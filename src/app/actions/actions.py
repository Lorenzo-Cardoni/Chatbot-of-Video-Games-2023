...
GITHUB_LINK = "https://github.com/Lorenzo-Cardoni/Chatbot-of-Video-Games-2023/blob/main/assets/img/"
...
class GetTeamGames(Action):
    ...
    def run(self,
        ...
        if similar_team is not None and similar_team.lower() != team.lower():
            msg += f"Didn't find {team}, searching for {similar_team} instead.\n"
            team = similar_team
        ...
        if result is not None and not result.empty:
            ...
            msg += f'No game made by {team} was found.\n'
            slots=tracker.slots
            msg += f'{slots}'
...
class GetGamesRecommendaton(Action):
    ...
    def run(self,
        ...
        if teams and teams != TEAMS_DEFAULT_NEGATIVE:
            result = filter_df_by_column_batch(result, teams, "Team", msg)

        if genres and genres != GENRES_DEFAULT_NEGATIVE:
...
class ValidateGamesRecommendationForm(FormValidationAction):
    ...
    def validate_genres(
        ...
        if tracker.get_slot("genres") != GENRES_DEFAULT_NEGATIVE:
            genres = list(set(tracker.get_slot('genres')))
            dispatcher.utter_message(text=f"Ok, I will filter by {genres}")
            return {"genres": genres, "genres_filter": True}
        else:
            return {"genres": GENRES_DEFAULT_NEGATIVE, "genres_filter": False}
    ...
    def validate_teams(
        ...
        if not tracker.get_slot("teams_filter"):
            ...
            teams = list(set(tracker.get_slot('teams')))
            dispatcher.utter_message(text=f"Ok, I will filter by {teams}")
            return {"teams": teams}
...
class AskForTeamsFiler(Action):
    ...
    def run(self,
            ...
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        dispatcher.utter_message(text="Would you like to filter by dev team?",
                                 buttons=[
...
class ActionResetSlots(Action):
    ...
    def run(self,
            ...
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        return [SlotSet("title", None), SlotSet("genres_filter", None),SlotSet("genres", None),SlotSet("teams_filter", None),SlotSet("teams", None)]
